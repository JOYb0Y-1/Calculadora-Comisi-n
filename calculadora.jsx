...
/* ── Material Icon Component (font-based) ──────────────────── */
function Icon({ name, size = 20, color = "currentColor", style = {} }) {
  return (
    <span
      className="material-icons-outlined"
      style={{ fontSize: size, color, verticalAlign: "middle", lineHeight: 1, ...style }}
    >
      {name}
    </span>
  );
...
function TierBadge({ tier, compact }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: compact ? "2px 8px" : "4px 12px", borderRadius: 6, fontSize: compact ? 10 : 11, fontWeight: 700, color: c.text, background: c.bg, border: `1.5px solid ${c.border}`, letterSpacing: "0.04em", whiteSpace: "nowrap" }}>
      {TIERS[tier].label}
    </span>
  );
}
...
function Stat({ icon, label, value, accent, mono }) {
  return (
    <div style={{ textAlign: "center", padding: "12px 8px" }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 800, color: accent || "#0F172A", fontFamily: mono ? "'JetBrains Mono', monospace" : "inherit", letterSpacing: "-0.02em", lineHeight: 1.2 }}>
        {value}
      </div>
      <div style={{ fontSize: 10, fontWeight: 600, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
...
function ResultCard({ data }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden", transition: "box-shadow 0.3s", boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
      {/* Card header */}
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #F1F5F9", display: "flex", alignItems: "center", justifyContent: "space-between", background: accentBg }}>
      ...
      <div style={{ padding: "16px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16, background: "#F8FAFC", borderRadius: 10, padding: "4px 0" }}>
          ...
          <Stat icon="payments" label="Comisión" value={`S/ ${fmt(data.totalCommission)}`} accent={accent} mono />
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "#64748B", display: "flex", alignItems: "center", gap: 4 }}>
              <Icon name="flag" size={14} color="#94A3B8" />
              Meta S/ {fmt(META_ASESOR)}
            </span>
            <span style={{ fontSize: 11, fontWeight: 700, color: data.metaPercent >= 100 ? "#16A34A" : "#D97706" }}>
              {data.metaPercent.toFixed(1)}%
            </span>
          </div>
          <div style={{ height: 6, background: "#F1F5F9", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${Math.min(data.metaPercent, 100)}%`, background: data.metaPercent >= 100 ? "linear-gradient(90deg, #16A34A, #4ADE80)" : `linear-gradient(90deg, ${accent}, ${accent}BB)`, borderRadius: 3, transition: "width 0.6s cubic-bezier(0.4,0,0.2,1)" }} />
          </div>
        </div>
        ...
        {/* Toggle detail */}
        <button onClick={() => setOpen(!open)} style={{ width: "100%", padding: "10px", background: open ? "#F8FAFC" : "#fff", border: "1px solid #E2E8F0", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600, color: "#64748B", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.2s" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "#F8FAFC", position: "sticky", top: 0 }}>
                  <th style={{ padding: "10px 12px", textAlign: "left", color: "#94A3B8", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em" }}>N°</th>
                  ...
                  <th style={{ padding: "10px 12px", textAlign: "right", color: "#94A3B8", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em" }}>Comisión</th>
                </tr>
              </thead>
              <tbody>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
        ...
        <h3 style={{ fontSize: 15, fontWeight: 800, color: "#0F172A", margin: 0 }}>Comparación Flujo vs Saldo</h3>
      </div>
            ...
            <tr>
              ...
              <th colSpan={3} style={{ padding: "12px 14px", textAlign: "center", fontSize: 10, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: "0.08em", background: "#FFFBEB", borderBottom: "2px solid #FDE68A" }}>
                <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                  <Icon name="trending_up" size={14} color="#D97706" /> Comisión Flujo
                </span>
              </th>
              <th colSpan={3} style={{ padding: "12px 14px", textAlign: "center", fontSize: 10, fontWeight: 700, color: "#2563EB", textTransform: "uppercase", letterSpacing: "0.08em", background: "#EFF6FF", borderBottom: "2px solid #BFDBFE" }}>
                <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                  <Icon name="savings" size={14} color="#2563EB" /> Comisión Saldo
                </span>
              </th>
            </tr>
            <tr style={{ borderBottom: "1px solid #E2E8F0" }}>
              <th style={{ padding: "8px 14px" }} />
              {["Tasa", "Clientes", "Total"].map((h) => (
                <th key={`f-${h}`} style={{ padding: "8px 10px", textAlign: "center", fontSize: 10, color: "#94A3B8", fontWeight: 600 }}>{h}</th>
              ))}
              {["Tasa", "Clientes", "Total"].map((h) => (
                <th key={`s-${h}`} style={{ padding: "8px 10px", textAlign: "center", fontSize: 10, color: "#94A3B8", fontWeight: 600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "24px 16px 48px" }}>
        ...
        <div className="fade-up" style={{ background: "#fff", borderRadius: 14, border: "1px solid #E2E8F0", overflow: "hidden", marginBottom: 24, boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
          <div style={{ padding: "16px 24px", borderBottom: "1px solid #F1F5F9", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <h2 style={{ fontSize: 14, fontWeight: 800, color: "#0F172A", margin: 0 }}>Parámetros de Simulación</h2>
          </div>
          ...
          <div style={{ padding: 24 }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                <Icon name="track_changes" size={14} color="#94A3B8" />
                Meta de comisión del asesor
              </label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                  <Icon name="swap_horiz" size={14} color="#94A3B8" />
                  Tipo de comisión
                </label>
                <div style={{ display: "flex", background: "#F1F5F9", borderRadius: 10, padding: 3, border: "1px solid #E2E8F0" }}>
                  {[
                    { key: "flujo", label: "Flujo", icon: "trending_up" },
                    { key: "saldo", label: "Saldo", icon: "savings" },
                  ].map((t) => (
                    <button key={t.key} onClick={() => setCommType(t.key)} style={{ flex: 1, padding: "10px 12px", fontSize: 13, fontWeight: 700, border: "none", borderRadius: 8, background: commType === t.key ? "#0F172A" : "transparent", color: commType === t.key ? "#fff" : "#64748B", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                      <Icon name={t.icon} size={16} color={commType === t.key ? "#fff" : "#94A3B8"} />
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                  <Icon name="filter_list" size={14} color="#94A3B8" />
                  Rango específico
                </label>
              ...
              {TIERS.map((t, i) => (
                <div key={i} style={{ background: "#fff", borderRadius: 8, padding: "12px 14px", border: "1px solid #E2E8F0", textAlign: "center" }}>
                  ...
                  <div style={{ marginTop: 6, fontSize: 11, color: "#94A3B8", fontWeight: 500 }}>
                    S/ {fmt(t.min)} – {t.max === Infinity ? "a más" : `S/ ${fmt(t.max)}`}
                  </div>
        ...
        <div className="fade-up" style={{ animationDelay: "0.1s" }}>
          ...
          <p style={{ fontSize: 12, color: "#94A3B8", marginBottom: 16, fontWeight: 500, paddingLeft: 30 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: results.length === 1 ? "1fr" : "repeat(auto-fit, minmax(290px, 1fr))", gap: 16, marginBottom: 28 }}>
            {results.map((r) => (
              <ResultCard key={`${r.tierIndex}-${commType}-${target}`} data={r} />
            ))}
          </div>
        </div>
